...
const PostDetail = ({ post } : any) => {
  const getContentFragment = (index :any, text:any, obj:any, type:any) => {
    ...
    switch (type) {
      case 'heading-three':
        return <h3 key={index} className="text-xl font-semibold mb-4">{modifiedText.map((item: string | number | boolean | React.ReactElement<any, string | React.JSXElementConstructor<any>> | React.ReactFragment | React.ReactPortal | null | undefined, i: React.Key | null | undefined) => <React.Fragment key={i}>{item}</React.Fragment>)}</h3>;
      case 'paragraph':
